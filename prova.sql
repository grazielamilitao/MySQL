...
#Prova Intermediaria Prática de BD II - 3ª etapa
#Nome da Dupla: 
...
insert into Conta_Corrente values (null, 45650, '2013-01-01', 0.00, 1, 6);
insert into Conta_Corrente values (null, 46897, '2014-01-30', 0.00, 1, 7);
insert into Conta_Corrente values (null, 47487, '2014-06-06', 0.00, 1, 8);
insert into Conta_Corrente values (null, 48456, '2015-04-21', 0.00, 1, 9);
insert into Conta_Corrente values (null, 280541, '2016-12-31', 0.00, 3, 10);
insert into Conta_Corrente values (null, 280191, '2016-12-31', 0.00, 3, 11);
...
delimiter $$
create procedure inserirDepósito(valor_dep float, data_dep date, tipo_dep varchar (100), cod_cc_fk int)
begin
	if(valor_dep>0)then
		if(tipo_dep='Cheque')then
			if(valor_dep<=2000)then
				insert into deposito values (null, valor_dep, data_dep, tipo_dep, cod_cc_fk);
             end if;
		else
			insert into deposito values (null, valor_dep, data_dep, tipo_dep, cod_cc_fk);
        end if;
...
delimiter $$
create trigger positivarSaldo after insert on deposito for each row
begin
	update Conta_Corrente set saldo_cc= saldo_cc+new.valor_dep where cod_cc= new.cod_cc_fk;
end $$ delimiter ;

call inserirDepósito(540,'2018-09-04','Dinheiro',1);

delimiter $$
create procedure inserirSaque(valor_saq float, data_saq date, local_saq varchar (100), cod_cc_fk int)
begin
	declare saldo double;
    select saldo_cc from conta_corrente where cod_cc=cod_cc_fk into saldo;
	if(saldo>=valor_saq)then
		insert into saque values (null, valor_saq, data_saq, local_saq, sysdate(), cod_cc_fk);
        select 'Saque realizado com sucesso!' as msg;
	else 
		 select 'Saldo da conta é insuficiente!' as msg;
	end if;
end $$ delimiter ;

delimiter $$
create trigger negativarSaldo after insert on saque for each row
begin
	update conta_corrente set saldo_cc=saldo_cc-new.valor_saq where cod_cc=new.cod_cc_fk;
end $$ delimiter ;

call inserirSaque(500,'2018-10-20', 'Bando do Brasil', 1);